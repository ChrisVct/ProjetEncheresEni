...
public class EnchereDAOJdbcImpl implements DAOEnchere {
		...
		List<Enchere> listeEncheres = new ArrayList<>();
			try(Connection cnx = ConnectionProvider.getConnection()) {
				...
				while (rs.next()) {
					...
					Enchere enchere = new Enchere(montant_enchere, new Utilisateur(noUtilisateurAcheteur),
										new Article(noArticle,nom_article, dateFinEncheres, statut, new Categorie(categorie), new Utilisateur(pseudoVendeur)));
					listeEncheres.add(enchere);
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public List<Enchere> selectAllFinWinnedById(int id) throws BusinessException {
		List<Enchere> listeEncheres = new ArrayList<>();
			try(Connection cnx = ConnectionProvider.getConnection()) {
				PreparedStatement pstmt = cnx.prepareStatement(SELECT_FIN_WINNED_BY_ID);
				pstmt.setInt(1, id);
				ResultSet rs = pstmt.executeQuery();
				while (rs.next()) {
					String nom_article = rs.getString("nom_article");
